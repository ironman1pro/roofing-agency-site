...
- **Caching**: `/assets/*` and `/portfolio/thumbs/*` are cached for a year (see `public/_headers`). Pages link them with a `?v=` number, so **whenever you edit a CSS/JS/image file there, bump its `?v=` in every page that links it** (e.g. `site.css?v=4` → `?v=5`), or visitors keep the old copy. All global styles live in one file, `public/assets/site.css`.
